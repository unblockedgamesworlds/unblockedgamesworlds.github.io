...
// 		console.log("My sq: "+SQUARES);
		UNITS = sudoku._get_all_units(ROWS, COLS);
// 		console.log("My u: "+UNITS);
		SQUARE_UNITS_MAP= sudoku._get_square_units_map(SQUARES, UNITS);
// 		console.log("My squ: "+SQUARE_UNITS_MAP);
		SQUARE_PEERS_MAP= sudoku._get_square_peers_map(SQUARES,SQUARE_UNITS_MAP);
// 		console.log("My spu: "+SQUARE_PEERS_MAP);
    }
...
//             difficulty = DIFFICULTY[difficulty] || DIFFICULTY.easy;
			difficulty = DIFFICULTY[difficulty.toLowerCase()]|| DIFFICULTY.easy;
// 			console.log(difficulty);
        }
    ...
    sudoku._get_all_units = function(srows, scols){
        /* Return a list of all units(rows, cols, boxes)
        */
        var units = [];

		var rows=srows.split('');
		var cols=scols.split('');
        // Rows
        for(var ri in rows){
...
// 		console.log(units);
    ...
    sudoku.validate_board = function(vboard){
        /* Return if the given `board` is valid or not. If it's valid, return
        true. If it's not, return a string of the reason why it's not.
        */
        
		var board=vboard.split('');
...
const scriptsInEvents = {
	...
	async Main_Event26_Act4(runtime, localVars)
	{
		runtime.globalVars.gen = sudoku.generate(runtime.globalVars.diff);
	},

	async Main_Event26_Act5(runtime, localVars)
	{
		runtime.globalVars.sol = sudoku.solve(runtime.globalVars.gen);
	},

	async Main_Event27_Act4(runtime, localVars)
	{
		runtime.globalVars.gen = sudoku.generate(runtime.globalVars.diff);
	},

	async Main_Event27_Act5(runtime, localVars)
	{
		runtime.globalVars.sol = sudoku.solve(runtime.globalVars.gen);
	},

	async Main_Event125_Act3(runtime, localVars)
	{
		console.log('check');
	},

	async Main_Event129_Act2(runtime, localVars)
	{
		runtime.globalVars.solb = sudoku.solve(runtime.globalVars.sola);
	}
